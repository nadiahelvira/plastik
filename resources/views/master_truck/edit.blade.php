        ...
        <div class="row">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{($tipx=='new')? url('/truck/store/') : url('/truck/update/'.$header->NO_ID ) }}" method="POST" name ="entri" id="entri" >
  
                      @csrf
						
                        <ul class="nav nav-tabs">
                            <!-- <li class="nav-item active">
                                <a class="nav-link active" href="#truckInfo" data-toggle="tab">Pegawai Info</a>
                            </li> -->
                            <!-- <li class="nav-item">
						...
						<div class="mt-3 col-md-12 form-group row">
							<div class="col-md-4">
								<button type="button" id='TOPX'  onclick="location.href='{{url('/truck/edit/?idx=' .$idx. '&tipx=top')}}'" class="btn btn-outline-primary">Top</button>
								<button type="button" id='PREVX' onclick="location.href='{{url('/truck/edit/?idx='.$header->NO_ID.'&tipx=prev&kodex='.$header->KODE )}}'" class="btn btn-outline-primary">Prev</button>
								<button type="button" id='NEXTX' onclick="location.href='{{url('/truck/edit/?idx='.$header->NO_ID.'&tipx=next&kodex='.$header->KODE )}}'" class="btn btn-outline-primary">Next</button>
								<button type="button" id='BOTTOMX' onclick="location.href='{{url('/truck/edit/?idx=' .$idx. '&tipx=bottom')}}'" class="btn btn-outline-primary">Bottom</button>
							</div>
							<div class="col-md-5">
								<button type="button" id='NEWX' onclick="location.href='{{url('/truck/edit/?idx=0&tipx=new')}}'" class="btn btn-warning">New</button>
								<button type="button" id='EDITX' onclick='hidup()' class="btn btn-secondary">Edit</button>                    
								<button type="button" id='UNDOX' onclick="location.href='{{url('/truck/edit/?idx=' .$idx. '&tipx=undo' )}}'" class="btn btn-info">Undo</button> 
								<button type="button" id='SAVEX' onclick='simpan()'   class="btn btn-success" class="fa fa-save"></i>Save</button>

							</div>
							<div class="col-md-3">
								<button type="button" id='HAPUSX'  onclick="hapusTrans()" class="btn btn-outline-danger">Hapus</button>
								<!-- <button type="button" id='CLOSEX'  onclick="location.href='{{url('/truck' )}}'" class="btn btn-outline-secondary">Close</button> -->
...
<script>
	...
	// 	{
	// 		window.location ="{{url('/truck/delete/'.$header->NO_ID )}}'";
	// 		//return true;
	// 	} 
	// 	return false;
	// }
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				Swal.fire({
	            	...
	            	loc = "{{ url('/truck/delete/'.$header->NO_ID) }}"  ;
	...
	function CariBukti() {
		...
		var cari = $("#CARI").val();
		var loc = "{{ url('/truck/edit/') }}" + '?idx={{ $header->NO_ID}}&tipx=search&kodex=' +encodeURIComponent(cari);
		window.location = loc;
	...
	function cekTruk(kode) {
		$.ajax({
			type: "GET",
			url: "{{url('truck/cektruck')}}",
            async: false,
			data: ({ KODE: kode, }),
	...
	function simpan() {
        ...
        (hasilCek==0) ? document.getElementById("entri").submit() : alert('Truk '+$('#KODE').val()+' sudah ada!');
