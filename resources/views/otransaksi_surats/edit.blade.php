...
//////////////////////////////////////////////////////////////////
	...
	chooseDo = function(NO_BUKTI, KODEP, NAMAP, RING, KOM,  NO_SO, KD_BRG,NA_BRG,SATUAN,QTY,NO_ID,HARGA, TYPE_KOM, KOM, TKOM, TOTAL_TKOM ){
		$("#NO_DO").val(NO_BUKTI);
		$("#KODEP").val(KODEP);
		$("#NAMAP").val(NAMAP);
		$("#RING").val(RING);
...
//	    $("#CLOSEX").attr("disabled", true);
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				Swal.fire({
	            	...
	            	loc = "{{ url('/surats/delete/'.$header->NO_ID) }}" + '?flagz=' + encodeURIComponent(flagz) + 
	...
	function closeTrans() {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
	        	loc = "{{ url('/surats/') }}" + '?flagz=' + encodeURIComponent(flagz) + '&golz=' + encodeURIComponent(golz) ;
				window.location = loc ;
			} else {
				Swal.fire({
					icon: 'info',
