...
//	    $("#CLOSEX").attr("disabled", true);
			...
			if (result.isConfirmed) {
				...
				Swal.fire({
	            	...
	            	loc = "{{ url('/so/delete/'.$header->NO_ID) }}" + '?flagz=' + encodeURIComponent(flagz) + 
						  '&golz=' + encodeURIComponent(golz) + '&typez=' + encodeURIComponent(typez) ;
	...
	function closeTrans() {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
	        	loc = "{{ url('/so/') }}" + '?flagz=' + encodeURIComponent(flagz) + '&golz=' + encodeURIComponent(golz) + '&typez=' + encodeURIComponent(typez)  ;
				window.location = loc ;
			} else {
				Swal.fire({
					icon: 'info',
